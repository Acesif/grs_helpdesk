    ...
    methods: {
        ...
        uploadFile(e) {
            ...
            reader.onload = function (event) {
                ...
                // Add preview URL to the ticket attachments
                self.ticket.attachments.push({
                    file: file,
                    preview: preview // Use the preview URL here
                });
            };
            ...
            axios.post(
                ...
                const index = self.ticket.attachments.findIndex(att => att.file === file);
                if (index !== -1) {
                    self.ticket.attachments[index] = {...self.ticket.attachments[index], ...response.data};
                }
            }).catch(function () {
                self.loading.file = false;
                self.uploadingFileProgress = 0;
